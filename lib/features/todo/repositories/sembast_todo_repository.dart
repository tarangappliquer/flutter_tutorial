...
import 'package:flutter_tutorial/features/todo/repositories/todo_repository.dart';
import 'package:flutter_tutorial/features/todo/todo_status.sealed.dart';
...
class SembastRepository extends _$SembastRepository implements TodoRepository {
  ...
  Stream<List<Todo>> build(Database database) {
    ...
    return query.onSnapshots(database).map((snapshots) {
      if (snapshots.isEmpty) {
        ...
        return snapshots.map((todoSnapShot) {
  ...
  Future<TodoStatus> addTodo(Todo todo) async {
    await database.transaction((txn) async {
      await _store.add(txn, todo.toJson());
    });
  ...
  Future<TodoStatus> updateTodoById(String todoId, Todo todo) async {
    ...
    Map<String, dynamic>? value = await _store
        .record(intTodoId)
        .update(database, todoJson);
